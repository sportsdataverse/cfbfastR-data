...
def card(pkg, cfg, suffix=None, sub=None):
    """One 1280x640 card. `suffix` two-tones the wordmark (e.g. '-data')."""
    motif = MOTIFS[cfg["motif"]](cfg["line"], cfg["seed"])
    word = pkg + (suffix or "")
    # DejaVu Sans Bold advances ~0.62em; keep the wordmark inside 1040px
    size = fit(word, (150, 1040), 0.62, 64)
    if suffix:
        mark = f'<tspan fill="#FFFFFF">{esc(pkg)}</tspan><tspan fill="{cfg["seed"]}">{esc(suffix)}</tspan>'
    else:
        mark = f'<tspan fill="#FFFFFF">{esc(pkg)}</tspan>'
    tag = sub or cfg["tagline"]
    tsize = fit(tag, (30, 1080), 0.72, 18)
  ...
  <!--
    ...
    the wordmark, a tagline, and the gold divider + diamond that every card in
    the family shares. Generated by make_cards.py; edit the config there and
    re-render rather than hand-editing this file.
  -->
  <defs>
  ...
  <text x="640" y="352" text-anchor="middle" font-family="{FONT}"
        font-size="{size}" font-weight="700"
        stroke="{cfg["bot"]}" stroke-width="16" paint-order="stroke">{mark}</text>

  <text x="640" y="430" text-anchor="middle" font-family="{FONT}"
        font-size="{tsize}" font-weight="600" fill="{cfg["seed"]}"
        letter-spacing="3">{esc(tag)}</text>

  <rect x="430" y="470" width="170" height="4" fill="{GOLD}"/>
  <rect x="680" y="470" width="170" height="4" fill="{GOLD}"/>
  <polygon points="640,454 658,472 640,490 622,472" fill="{GOLD}"/>
  <text x="640" y="522" text-anchor="middle" font-family="{FONT}"
        font-size="26" font-weight="400" fill="#8FA3B5"
